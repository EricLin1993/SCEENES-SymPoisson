...
%% Date: 2024-01-26
%% This script is to reproduce 1H-1H symmetrical spectra reconstruction using SCREENES-soft.
%% The spectra data are obtained from 128*128 1H-1H projection of the COSY-TOCSY spectrum of quinine.
%%
clear classes
clear,close all,clc
addpath('data')
...
figure,contour(ppm1,ppm2,(Spec2D_Ideal),(linspace(0,1,100)))
...
figure,contour(ppm1,ppm2,Rec_X_Symetric,linspace(0,1,100));
set (gca,'xdir','reverse','ydir','reverse','box','on','linewidth',1.5,'fontsize',14,'fontname','Calibri','fontweight','bold');
...
[pks,locs,w,p] = findpeaks(diag(RecS),'minpeakheight',0.1); % find diagonal peaks
...
bwm = ((IdeS_RevDiag+IdeS_RevDiag.')/2>0.03);
...
R1 = RecS;
R1(R1<0.03)=0;
[temp_IRec,temp_locRec] = findpeaks(R1(:),'minpeakheight',0.005);
pRec = 0*RecS;
pRec(temp_locRec) = 1;
